...
- 💾 **一鍵備份與還原** — 輕鬆匯出與匯入 LDIF 檔案
- 🌐 **多語系支援 (i18n)** — 內建即時切換繁體中文與英文 UI
- 🔍 **進階搜尋過濾器** — 視覺化的階層查詢過濾器，支援複雜的 LDAP AND/OR 語法
- ⚡ **批次操作** — 支援介面上多選使用者與群組進行大量刪除

![LDAP-in-a-Box 儀表板](docs/frontend-dashboard.png)

![LDAP-in-a-Box 樹狀目錄瀏覽器](docs/frontend-ui.png)

![LDAP-in-a-Box 新增使用者](docs/frontend-adduser.png)
...
│   + JWT 認證              │  389   │  + 自訂 Schema (isVPN, IsMailMonitor) │
│                          │        │  + 初始 LDIF 種子資料      │
...
| POST | `/api/v1/backup/restore` | 匯入 LDIF 還原 |
...
- **郵件伺服器驗證**：透過 `IsMailMonitor` 自訂屬性管控特定信件寄發行為與審核流程
